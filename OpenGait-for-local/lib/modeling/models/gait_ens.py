...
class GaitPart(BaseModel):
    ...
    def build_network(self, model_cfg2):

        self.Backbone = self.get_backbone(model_cfg2['backbone_cfg'])
        head_cfg = model_cfg2['SeparateFCs']
        self.Head = SeparateFCs(**model_cfg2['SeparateFCs'])
        self.Backbone = SetBlockWrapper(self.Backbone)
        self.HPP = SetBlockWrapper(
            HorizontalPoolingPyramid(bin_num=model_cfg2['bin_num']))
...
class GaitSet(BaseModel):
    ...
    def build_network(self, model_cfg3):
        in_c = model_cfg3['in_channels']
        ...
        self.Head = SeparateFCs(**model_cfg3['SeparateFCs'])

        self.HPP = HorizontalPoolingPyramid(bin_num=model_cfg3['bin_num'])
...
class GaitEns(BaseModel):
    def __init__(self, GaitSet, GaitPart, GaitGL, *args, **kargs, ):
        super(GaitEns, self).__init__(*args, **kargs)
        self.GaitPart = GaitPart
    ...
    def forward(self):
        embs1 , logi1, labs1 = self.GaitGL(BaseModel)
        embs2 , labs2 = self.GaitPart(BaseModel)
        embs3, labs3, sils = self.GaitSet(BaseModel)

        embs = (embs1 + embs2 + embs3)/3
        labs = (labs1 + labs2 + labs3)/3
